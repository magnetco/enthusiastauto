...
import { MOBILE_CONTACT_MENU } from "@/lib/config/navigation";
import { useMobileMenu } from "./MobileMenuContext";

/**
 * Mobile contact submenu panel
 * Slides in from the right on top of the main menu
 */
export function MobileContactPanel() {
  const { isOpen, contactPanelOpen, closeContactPanel, closeMenu } =
    useMobileMenu();
  ...
  return (
    <Transition show={isOpen && contactPanelOpen}>
      <Dialog onClose={closeContactPanel} className="relative z-[60]">
          ...
          <Dialog.Panel className="fixed inset-y-0 left-0 flex w-full max-w-sm flex-col bg-[#0a0c10]">
            ...
            <nav className="flex-1 overflow-y-auto px-6 py-6">
              <ul className="space-y-2">
                {MOBILE_CONTACT_MENU.items.map((item) => (
                  <li key={item.href}>
                    <Link
                      href={item.href}
                      onClick={handleLinkClick}
