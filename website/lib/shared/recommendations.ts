...
/**
 * Shopify GraphQL query for products sorted by best selling
 */
const popularProductsQuery = `
  query getPopularProducts($query: String!, $first: Int!) {
...
/**
 * Get popular compatible parts for a vehicle, sorted by best selling
 *
 * Uses Shopify's BEST_SELLING sort to prioritize popular products:
 * 1. Generates compatibility tags from vehicle CMS fields (chassis, model, year)
 * 2. Searches Shopify for products with ANY of these exact tags, sorted by sales
 * 3. Returns top N products (configurable, default 8)
 *
 * Example: For "2013 BMW E92 M3", searches for best-selling products tagged with:
 * "E92", "M3", "BMW E92", "BMW M3", "E92 M3", etc.
 *
 * Uses 5-minute in-memory cache to reduce API calls
 */
export async function getPopularCompatibleParts(
  ...
  // Check cache first
  const cacheKey = `popular-compatible-parts:${vehicle.slug.current}:${limit}`;
  const cached = memoryCache.get<Product[]>(cacheKey);
  if (cached) {
    return cached;
  }
  ...
  try {
    ...
    console.log(
      `[PopularParts] Searching for ${vehicle.listingTitle}`,
      `\nGenerated tags:`,
      compatibilityTags,
    );
    ...
    const searchQuery = tagQueries;
    console.log(`[PopularParts] Shopify query:`, searchQuery);

    type PopularProductsQueryType = {
      data: {
        products: {
          edges: Array<{
            node: {
    ...
    const res = await shopifyFetch<PopularProductsQueryType>({
      query: popularProductsQuery,
      variables: { query: searchQuery, first: limit + 5 }, // Fetch extra to account for filtering
    });
    ...
    // Transform to Product type with images array
    const products: Product[] = rawProducts
      .filter((p) => p.featuredImage) // Only include products with images
      .map((p) => ({
        id: p.id,
        handle: p.handle,
        availableForSale: p.availableForSale,
    ...
    console.log(
      `[PopularParts] Found ${products.length} popular compatible products`,
    );
    ...
    console.error("Error fetching popular compatible parts:", error);
    return [];
  }
}
